  ...
  methods: {
    ...
    async create () {
      ...
      if (this.addValidater().status === 400) {
        ...
        data.append('food_id', this.$route.params.id)
        this.loader = true
        await this.axios
          .post('foodObjectsRestaurant', data)
          .then(() => {
            this.getAllItems()
            this.loader = false
    ...
    addToAll () {
      let data = new FormData()
        ...
        this.loader = true
        this.axios
          .post('add/all/restaurant/food', data)
          .then(() => {
            this.getAllItems()
            this.loader = false
          })
    ...
    async update () {
      ...
      Object.keys(this.formEditRestaurant).forEach((key) => {
        ...
        this.loader = true
        await this.axios
            .post(`foodObjectsRestaurant/${this.formEdit.id}`, data)
            .then(() => {
                this.getAllItems()
    ...
    getAllItems () {
      ...
      this.axios
        .get(
          `all/restaurant/food/${this.$route.params.id}?take=10`
        )
        .then((res) => {
    ...
    deleteItem () {
      this.loader = true
      this.axios
        .delete(`foodObjectsRestaurant/${this.id}`)
        .then((res) => {
          this.getAllItems()
